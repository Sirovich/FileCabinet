    ...
    public class DeleteCommandHandler : ServiceCommandHandlerBase
    {
        ...
        private void ParseArguments(string parameters)
        {
            ...
            else
            {
                ...
                {
                    if (!this.FillFields(ref recordId, ref recordFirstName, ref recordLastName, ref recordDate, ref recordSex, ref recordWeight, ref recordHeight, pair.First(), pair.Last().Trim('\'')))
                    {
                        return;
                    }

                    if (recordId != null && recordId.Count > 1 && type.Equals("and", StringComparison.InvariantCulture))
                    {
                        Console.WriteLine("1488");
                        return;
                    }
                }
        ...
        private bool FillFields(ref List<int> recordId, ref List<string> recordFirstName, ref List<string> recordLastName, ref List<DateTime> recordDate, ref List<char> recordSex, ref List<decimal> recordWeight, ref List<short> recordHeight, string key, string value)
        {
